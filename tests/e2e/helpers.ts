...
const E2E_REPORT_JSONL_PATH = resolve(
  process.cwd(),
...
const globalState = globalThis as typeof globalThis & {
  __elasticsearchfsReportInitialized?: boolean;
};
...
export function hasProfileEnv(profile: Profile): boolean {
  return Boolean(
    process.env.ELASTICSEARCH_URL &&
      process.env[`ELASTICSEARCH_API_KEY_${profile}`],
  );
}
...
export async function createElasticSession(
  profile: Profile,
): Promise<ElasticSession> {
